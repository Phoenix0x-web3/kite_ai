...
KITE_BRIDGE_ROUTER = RawContract(title="KiteBridgeRouter", address="0x0BBB7293c08dE4e62137a557BC40bc12FA1897d6", abi=ABI)
KITE_SWAP_ROUTER_NATIVE = RawContract(
    title="KiteSwapRouter", address=Web3.to_checksum_address("0x04CfcA82fDf5F4210BC90f06C44EF25Bf743D556"), abi=ABI
)
...
class Contracts:
    KITE = RawContract(title="KITE", address=ZERO_ADDRESS, abi=DefaultABIs.Token)

    USDT = RawContract(title="USDT", address=Web3.to_checksum_address("0x0fF5393387ad2f9f691FD6Fd28e07E3969e27e63"), abi=DefaultABIs.Token)
...
class KiteOnchain(Base):
    ...
    async def _build_instructions(
        self, receiver: str, swap_native_to_erc20: bool, token_in: str, token_out: str, amount_out: TokenAmount, amount_out_min: TokenAmount
    ) -> tuple:
    ...
    async def deposit(self, to: str, amount: TokenAmount) -> str:
        tx = await self.client.transactions.sign_and_send(TxParams(to=Web3.to_checksum_address(to), value=amount.Wei, data="0x"))
    ...
    async def get_pool_address(self, from_token: RawContract, to_token: RawContract):
        ...
        data = TxArgs(tokenA=from_token.address, tokenB=to_token.address).tuple()
        ...
        try:
            ...
            if pool_address == "0x0000000000000000000000000000000000000000":
                ...
                return RawContract(title="POOL", address=pool_address, abi=ABI)
    ...
    async def _swap(self, amount: TokenAmount, from_token: RawContract, to_token: RawContract, slippage: float = 3.0, fee: int = 500):
        ...
        if from_token_is_kite:
            from_token = Contracts.WKITE

        to_token_is_kite = to_token.address.upper() == Contracts.KITE.address.upper()
        if to_token_is_kite:
            to_token = Contracts.WKITE
        ...
        args = (from_token.address, amount.Wei, instructions)

        data = c.encode_abi("initiate", args=args)

        if not from_token_is_kite:
            ok = await self.approve_interface(token_address=from_token.address, spender=c.address, amount=None)
            if not ok:
                return "Failed | approve"

            await asyncio.sleep(random.randint(2, 5))

        tx_params = TxParams(to=c.address, data=data, value=int(amount.Wei) if from_token_is_kite else 0)
    ...
    async def controller(self, action: str):
        tokens = [Contracts.KITE, Contracts.USDT]
        ...
        if action == "bridge":
            return await self.bridge_send(token=from_token, dest_chain_id=84532, amount=amount)
    ...
    async def check_bridge_status(self):
        ...
        r = await self.session.get(url="https://bridge-backend.prod.gokite.ai/check-interaction", params=params, headers=headers)

        return r.json().get("data").get("has_interacted")

    @controller_log("Bridge send")
    async def bridge_send(self, token: RawContract | str, dest_chain_id: int, amount: TokenAmount) -> str:
        ...
        tx = await self.client.transactions.sign_and_send(TxParams(to=c.address, data=data, value=amount.Wei if is_native else 0))
